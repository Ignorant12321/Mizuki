...
class l {
	...
	error(e, ...t) {
		this.shouldLog("error") &&
			console.error("[Live2D Widget][ERROR]", e, ...t);
	}
	warn(e, ...t) {
		this.shouldLog("warn") &&
			console.warn("[Live2D Widget][WARN]", e, ...t);
	}
	info(e, ...t) {
		this.shouldLog("info") && console.log("[Live2D Widget][INFO]", e, ...t);
	}
	trace(e, ...t) {
		this.shouldLog("trace") &&
			console.log("[Live2D Widget][TRACE]", e, ...t);
	}
}
...
class c {
	constructor(e, t = []) {
		...
		if ("string" == typeof i) (i.endsWith("/") || (i += "/"), (c = !0));
			...
			(o.endsWith("/") || (o += "/"),
				...
				(c = !0),
				a.warn(
					"apiPath option is deprecated. Please use cdnPath instead.",
				));
	...
	async loadTextureCache(e) {
		return (
			(await this.fetchWithCache(
				`${this.cdnPath}model/${e}/textures.cache`,
			)) || []
		);
	}
	...
	async loadNextModel() {
		((this.modelTexturesId = 0),
			this.useCDN
				? ((this.modelId =
						(this.modelId + 1) % this.modelList.models.length),
					...
					callback: () => {
						if (window.Asteroids)
							(window.ASTEROIDSPLAYERS ||
								(window.ASTEROIDSPLAYERS = []),
								window.ASTEROIDSPLAYERS.push(
									new window.Asteroids(),
								));
						else {
				...
				"switch-texture": {
					...
					callback: () => {
						let t = "",
							o = "";
						(s &&
							((t = s.message.changeSuccess),
							(o = s.message.changeFail)),
			...
			(function (t) {
				...
				(console.log("%c", a),
					...
					window.addEventListener("visibilitychange", () => {
						document.hidden ||
							i(t.message.visibilitychange, 6e3, 9);
					}));
			})(o),
			i(
				(function (e, t, s) {
					...
					if ("" === document.referrer || !s) return o;
					const i = new URL(document.referrer);
					return location.hostname === i.hostname
						? o
						: `${n(s, i.hostname)}<br>${o}`;
