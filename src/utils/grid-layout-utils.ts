...
export function calculateGridLayout(
	...
	if (desktopShowLeftSidebar && desktopShowRightSidebar) {
		desktopGridCols =
			"lg:grid-cols-[var(--sidebar-column-width)_1fr_var(--sidebar-column-width)]";
	} else if (desktopShowLeftSidebar) {
		desktopGridCols = "lg:grid-cols-[var(--sidebar-column-width)_1fr]";
	} else if (desktopShowRightSidebar) {
		desktopGridCols = "lg:grid-cols-[1fr_var(--sidebar-column-width)]";
	}

	const gridCols = `
		${mobileShowSidebar ? "grid-cols-1" : "grid-cols-1"}
		${tabletAnySidebar ? "md:grid-cols-[var(--sidebar-column-width)_1fr]" : "md:grid-cols-1"}
		${desktopGridCols}
	`
		.trim()
		.replace(/\s+/g, " ");
	...
	const sidebarClass = `
		...
		${tabletShowLeftSidebar ? "md:block md:mb-4 md:max-w-[var(--sidebar-column-width)]" : "md:hidden"}
		${desktopShowLeftSidebar ? "lg:block lg:mb-4 lg:row-start-1 lg:row-end-2 lg:max-w-[var(--sidebar-column-width)] lg:col-start-1 lg:col-end-2" : "lg:hidden"}
	`
		.trim()
		.replace(/\s+/g, " ");
	...
	const rightSidebarClass = `
		onload-animation
		hidden
		${tabletShowRightSidebar ? "md:block md:mb-4 md:max-w-[var(--sidebar-column-width)]" : "md:hidden"}
		${desktopShowRightSidebar ? `lg:block lg:self-start lg:h-fit lg:mb-4 lg:max-w-[var(--sidebar-column-width)] ${desktopShowLeftSidebar ? "lg:col-start-3 lg:col-end-4" : "lg:col-start-2 lg:col-end-3"} lg:col-span-1` : "lg:hidden"}
